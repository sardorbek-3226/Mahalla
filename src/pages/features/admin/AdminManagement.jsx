import { useState } from 'react';
import { useForm } from 'react-hook-form';
import { useQuery, useMutation } from '@tanstack/react-query';
import { useTranslation } from 'react-i18next';
import toast from 'react-hot-toast';
import { HiOutlineShieldCheck, HiPlus, HiOutlineNoSymbol, HiOutlineUserPlus } from 'react-icons/hi2';
import PageHeader from '@/components/common/PageHeader';
import Card from '@/components/ui/Card';
import { Avatar, Input, Button, Badge, Modal, Skeleton, EmptyState } from '@/components/ui';
import { adminService } from '@/services/adminService';
import { queryClient } from '@/config/queryClient';
import { usePermissions } from '@/hooks/usePermissions';
import { ROLE_LABELS, ROLE_SCOPE_FIELD } from '@/constants/roles';
import { formatDate } from '@/utils/format';

const AdminManagement = () => {
  const { t } = useTranslation();
  const { user, creatableRoles, canManageRole } = usePermissions();
  const [open, setOpen] = useState(false);
  const { register, handleSubmit, watch, reset, formState: { errors } } = useForm({
    defaultValues: { role: creatableRoles[0] || '' },
  });
  const selectedRole = watch('role');
  const scopeField = ROLE_SCOPE_FIELD[selectedRole];

  const { data, isLoading } = useQuery({ queryKey: ['admins'], queryFn: () => adminService.listAdmins() });
  const admins = data?.items || data || [];

  const create = useMutation({
    mutationFn: (payload) =>
      adminService.createAdmin({
        ...payload,
        actor: user?.full_name,
        actor_role: user?.role,
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admins'] });
      queryClient.invalidateQueries({ queryKey: ['audit-logs'] });
      toast.success(t('admin.management.createSuccess'));
      reset();
      setOpen(false);
    },
    onError: (e) => toast.error(e.response?.data?.message || t('admin.management.createError')),
  });

  const suspend = useMutation({
    mutationFn: (id) => adminService.suspendAdmin(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admins'] });
      toast.success(t('admin.management.suspendSuccess'));
    },
  });

  const onSubmit = (values) => {
    // Privilege-escalation guard (frontend mirror; backend re-checks).
    if (!creatableRoles.includes(values.role)) {
      return toast.error(t('admin.management.noPermission'));
    }
    create.mutate(values);
  };

  const canCreate = creatableRoles.length > 0;

  return (
    <div>
      <PageHeader
        title={t('admin.management.title')}
        subtitle={t('admin.management.subtitle')}
        actions={
          canCreate && (
            <Button variant="gradient" leftIcon={<HiPlus className="h-4 w-4" />} onClick={() => setOpen(true)}>
              {t('admin.management.addAdmin')}
            </Button>
          )
        }
      />

      {!canCreate && (
        <Card className="mb-5 flex items-center gap-3 border-l-4 border-l-amber-400 p-4 text-sm text-gray-600 dark:text-gray-300">
          <HiOutlineShieldCheck className="h-6 w-6 shrink-0 text-amber-500" />
          {t('admin.management.noCreateNotice')}
        </Card>
      )}

      <Card className="p-0">
        {isLoading ? (
          <div className="space-y-2 p-4">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-16 rounded-xl" />)}</div>
        ) : admins.length === 0 ? (
          <EmptyState icon={HiOutlineShieldCheck} title={t('admin.management.empty')} />
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="border-b border-gray-100 text-left text-gray-400 dark:border-gray-800">
                <tr>
                  <th className="px-5 py-3 font-medium">{t('admin.management.columns.administrator')}</th>
                  <th className="px-5 py-3 font-medium">{t('admin.management.columns.role')}</th>
                  <th className="px-5 py-3 font-medium">{t('admin.management.columns.region')}</th>
                  <th className="px-5 py-3 font-medium">{t('admin.management.columns.created')}</th>
                  <th className="px-5 py-3 font-medium">{t('admin.management.columns.status')}</th>
                  <th className="px-5 py-3 text-right font-medium">{t('admin.management.columns.action')}</th>
                </tr>
              </thead>
              <tbody>
                {admins.map((a) => (
                  <tr key={a.id} className="border-b border-gray-50 last:border-0 dark:border-gray-800/50">
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2">
                        <Avatar name={a.full_name} size="xs" />
                        <div>
                          <p className="font-medium">{a.full_name}</p>
                          <p className="text-xs text-gray-400">{a.phone}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-3"><Badge tone="blue">{ROLE_LABELS[a.role] || a.role}</Badge></td>
                    <td className="px-5 py-3 text-gray-500">{a.scope}</td>
                    <td className="px-5 py-3 text-gray-500">{formatDate(a.created_at)}</td>
                    <td className="px-5 py-3">
                      <Badge tone={a.is_active ? 'green' : 'red'} dot>{a.is_active ? t('admin.management.active') : t('admin.management.suspended')}</Badge>
                    </td>
                    <td className="px-5 py-3 text-right">
                      {a.is_active && canManageRole(a.role) && (
                        <Button size="sm" variant="outline" leftIcon={<HiOutlineNoSymbol className="h-4 w-4" />} onClick={() => suspend.mutate(a.id)}>
                          {t('admin.management.suspend')}
                        </Button>
                      )}
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        )}
      </Card>

      <Modal open={open} onClose={() => setOpen(false)} title={t('admin.management.newAdmin')}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <Input label={t('admin.management.fullName')} leftIcon={<HiOutlineUserPlus className="h-4 w-4" />} error={errors.full_name?.message} {...register('full_name', { required: t('admin.management.fullNameRequired') })} />
          <Input label={t('admin.management.phone')} placeholder="+998…" error={errors.phone?.message} {...register('phone', { required: t('admin.management.phoneRequired') })} />
          <div>
            <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">{t('admin.management.role')}</label>
            <select className="input-base" {...register('role', { required: true })}>
              {creatableRoles.map((r) => (
                <option key={r} value={r}>{ROLE_LABELS[r]}</option>
              ))}
            </select>
          </div>
          {scopeField && (
            <Input
              label={t('admin.management.scopeAssignLabel', { scope: t(`admin.management.scopeLabels.${scopeField}`) })}
              placeholder={t('admin.management.scopeNamePlaceholder', { scope: t(`admin.management.scopeLabels.${scopeField}`) })}
              error={errors.scope?.message}
              {...register('scope', { required: t('admin.management.scopeRequired') })}
            />
          )}
          <Button type="submit" variant="gradient" className="w-full" loading={create.isPending}>
            {t('admin.management.create')}
          </Button>
        </form>
      </Modal>
    </div>
  );
};

export default AdminManagement;
